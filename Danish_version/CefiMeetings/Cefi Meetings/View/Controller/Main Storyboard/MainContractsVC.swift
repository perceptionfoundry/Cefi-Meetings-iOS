...
//     
    ...
    @IBAction func ListDisplayOption(_ sender: UIButton) {
        ...
        if sender.tag == 0{
            ...
            let value = self.contractFilter(option: "Approved")
...
//
            ...
            let value = self.contractFilter(option: "Booked")
...
//            print(value)
            ...
            let value = self.contractFilter(option: "Expired")
...
//            print(value)
    ...
    func contractFilter(option : String) -> [Contract]{
        ...
        if option == "Approved"{
            result = allContract.filter( {$0.contractStatus == "Approved" }).map({ return $0 })
            
        }
        else if option == "Booked"{
            result = allContract.filter( {$0.contractStatus == "Booked" }).map({ return $0 })
            
            
        }
        else if option == "Expired"{
            result = allContract.filter( {$0.contractStatus == "Expired" }).map({ return $0 })
...
//        print(userContract[indexPath.row].allPendingDocumentCounts)
        ...
        if userContract[indexPath.row].allPendingDocumentCounts! > 0 && userContract[indexPath.row].contractStatus == "Booked"{
