...
//        industryTF.text = contactDetail.industryType
        ...
        if let lati = contactDetail.lat, let longi = contactDetail.longField{
            lat = (lati as NSString).doubleValue
            long  = (longi as NSString).doubleValue
        }
        ...
        self.getAddressFromLatLon(pdblLatitude: "\(lat)", withLongitude: "\(long)") { (place) in
