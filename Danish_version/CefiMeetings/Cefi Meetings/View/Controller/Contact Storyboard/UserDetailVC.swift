...
//        industryCatergory.text = userDetail!.industryType
        ...
        getAddressFromLatLon(pdblLatitude: userDetail!.lat ?? "0.0", withLongitude: userDetail!.longField ?? "0.0") { (place) in
...
//            print(count)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if tableContent[indexPath.row].contractStatus == "Booked"{
        ...
        if tableContent[indexPath.row].allPendingDocumentCounts! > 0 && tableContent[indexPath.row].contractStatus == "Booked"{
